...
void OLED_WR_Byte(uint8_t dat, uint8_t cmd)
{
    ...
    if (cmd)
        ...
        OLED_DC_Clr();//表示写命令
    OLED_CS_Clr(); /* SPI时序：选中OLED设备 */

    //2.写数据：
    //本OLED使用的是只写SPI协议，舍弃了数据交换（读取）的过程。按SPI写数据时序进行数据写入即可
    for (i = 0; i < 8; i++)//时钟线拉低，主机放数据。时钟线拉高，从机读数据。没有交换数据，故长得很像I2C
    {
        OLED_SCLK_Clr();
        if (dat & 0x80)
            OLED_SDIN_Set();
...
void OLED_ShowCH(uint8_t x, uint8_t y, uint8_t chr, uint8_t size, uint8_t mode)
{
    ...
    for (t = 0; t < size; t++)
    {
        ...
        for (t1 = 0; t1 < 8; t1++)
        {
            if (temp & 0x80)//如果目前读到的8位数据的第一位为1
                OLED_DrawPoint(x, y, mode);//在对应坐标描点
            else
                OLED_DrawPoint(x, y, !mode);//在对应坐标不描点
            temp <<= 1;//读取8位数据的下一位
            y++;
            if ((y - y0) == size)//竖着写到头了（当size为12时，数组中列数为偶数的8位数据仅有前4位有效，后4位全为0）
            {
                y = y0;//竖着再从头开始
                x++;//横着加一格才能继续写
                break;
